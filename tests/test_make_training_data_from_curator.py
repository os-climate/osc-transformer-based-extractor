...
dummy_curator_data = pd.DataFrame(
    {"context": ["Lorem ipsum"], "kpi_id": [1], "label": [1]}
)
...
def test_make_training_data():
    ...
    make_training_data(
        curator_data_path_valid, kpi_mapping_path_valid, output_path_valid
    )
...
def test_invalid_kpi_id():
    ...
    dummy_curator_data_invalid_kpi = pd.DataFrame(
        {"context": ["Lorem ipsum"], "kpi_id": [999], "label": [1]}
    )
    ...
    with pytest.raises(KeyError):
        make_training_data(
            invalid_curator_data_path, kpi_mapping_path_valid, output_path_valid
        )
