...
from src.osc_transformer_based_extractor.fine_tune import (
    check_csv_columns,
    check_output_dir,
    fine_tune_model,
)
...
def test_check_csv_columns_missing_columns(tmp_path):
    ...
    df = pd.DataFrame(
        {
            "question": ["What is this?"],
            "wrong_column": ["This is a paragraph."],
            "label": [1],
        }
    )
    df.to_csv(csv_path, index=False)
...
def test_fine_tune_model(mock_trainer, tmp_path):
    ...
    mock_trainer_instance.predict.return_value = MagicMock(
        predictions=MagicMock(argmax=lambda axis: [0, 1]), label_ids=[0, 1]
    )
