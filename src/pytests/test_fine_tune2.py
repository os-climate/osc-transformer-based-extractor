...
def test_check_csv_columns_valid(tmp_path):
    ...
    csv_path = tmp_path / "test_valid.csv"import os
import pytest
import pandas as pd
from unittest.mock import patch, MagicMock
from src.osc_transformer_based_extractor.fine_tune import check_csv_columns, check_output_dir, fine_tune_model
...
@pytest.fixture
def mock_trainer():
    """Fixture to return a mock trainer instance."""
    return MockTrainer()

def test_check_csv_columns_valid(tmp_path):
    """Test check_csv_columns function with a valid CSV file."""
...
def test_check_csv_columns_missing_columns(tmp_path):
    """Test check_csv_columns function with a CSV file missing required columns."""
...
def test_check_output_dir_valid(tmp_path):
    """Test check_output_dir function with an existing directory."""
...
def test_check_output_dir_invalid(tmp_path):
    """Test check_output_dir function with a non-existent directory."""
    output_dir = tmp_path / "non_existent_output"

    with pytest.raises(ValueError, match=r".*does not exist.*"):
        check_output_dir(output_dir)

@patch('osc_transformer_based_extractor.fine_tune.Trainer', autospec=True)
